...
\documentclass[journal,compsoc,twocolumn,10pt,letterpaper,twoside]{IEEEtran}
...
%
\thanks{
  K. Moreland is with Sandia National Laboratories, PO Box 5800, MS 1326,
  Albuquerque, NM 87185-1326.  E-mail: kmorel@sandia.gov

  \vspace{1ex}
  \noindent Digital Object Identifier no. 10.1109/TVCG.2012.133.

  \noindent \copyright 2013 IEEE
}
}
...
\markboth{%
IEEE Transactions on Visualizations and Computer Graphics,~Vol.~19, No.~3,~March~2013%
}{%
Moreland: A Survey of Visualization Pipelines%
}

% The actual publication starts on page 367.
\setcounter{page}{367}


%% ``Fix'' borders to match the actual publication.

\setlength{\headsep}{0.15in}

% Set both the left and right border to be 26pt on each side (closer to
% what is in the actual paper) PLUS add another 0.1in on each side to
% compensate for an oddly thin page.
...
% This should set the top and bottom border to be 26pt.
\setlength{\topmargin}{-46pt}
...
\begin{itemize}
\item \keyterm{Modules} are functional units.  Each module has zero or more
  input ports that ingest data and an independent number of zero or more
  output ports that produce data.  The function of the module is fixed
  whereas data entering an input port typically change.  Data emitted from
  the output ports are the result of the module's function operating on the
  input data.
\item \keyterm{Connections} are directional attachments from the output
  port of one module to the input port of another module.  Any data emitted
  from the output port of the connection enter the input port of the
  connection.  Together modules and connections form the nodes and arcs,
  respectively, of a directional graph.  The dataflow network can be
  configured by defining connections and connections are arbitrary subject
  to constraints.
\item \keyterm{Execution} management is inherent in the pipeline.
  Typically there is a mechanism to invoke execution, but once invoked data
  automatically flows through the network.
\end{itemize}
...
\begin{figure*}[htbp]
  ...
  \subfloat[Update Information]{
    \includegraphics[scale=.9]{images/UpdateInformation}
    \label{fig:RegionPasses:UpdateInformation}
  } &
  \subfloat[Update Region]{
    \includegraphics[scale=.9]{images/UpdateRegion}
    \label{fig:RegionPasses:UpdateRegion}
  } &
  \subfloat[Update Data]{
    \includegraphics[scale=.9]{images/UpdateData}
...
\begin{itemize}
\item \keyterm{Extents} are valid index ranges for regular multidimensional
  arrays of data.  Extents allow a fine granularity in defining regions as
  sub-arrays within a larger array.
\item \keyterm{Pieces} are arbitrary collections of cells.  Pieces allow
  unstructured grids to be easily decomposed into discretionary regions.
\item \keyterm{Blocks} (or domains) represent a logical domain
  decomposition.  Blocks are similar to pieces in that they can represent
  arbitrary collections, but blocks are defined by the data set and their
  structures are considered to have some meaning.
\end{itemize}
...
\begin{figure*}[htbp]
  ...
  \caption{Concurrent execution with data parallelism.  Boxes represent
